...
class MobileExecutorService:
                                ...
                                try:
                                    if step_type in ['db_query', 'database']:
                                        try:
                                            ...
                                            url_val = MobileExecutorService.replace_vars(raw_conn, current_path_vars) or ""
                                    ...
                                    if step_type == 'e2e':
                                        try:
                                            ...
                                            resolved_step_data = json.loads(MobileExecutorService.replace_vars(step_data_str, current_path_vars))
                                            ...
                                            if step_data.get('type') == 'browser' and step_data.get('properties', {}).get('value'):
                                                ...
                                                if val and not val.startswith(('http://', 'https://')):
                                                    val = f"http://{val}"
                                                step_data['properties']['value'] = MobileExecutorService.sanitize_url_for_docker(val)
                                        ...
                                        url = MobileExecutorService.replace_vars(step_data.get('url', ''), current_path_vars)
                                        ...
                                        if method == 'PYTHON':
                                            ...
                                            url = MobileExecutorService.sanitize_url_for_docker(url)

                                            if MobileExecutorService.is_blocked_domain(url):
                                            ...
                                            headers = json.loads(MobileExecutorService.replace_vars(headers_json_str, current_path_vars))
                                            ...
                                            params = json.loads(MobileExecutorService.replace_vars(params_json_str, current_path_vars))
                                            ...
                                            if isinstance(body_raw, (dict, list)):
                                                ...
                                                body = MobileExecutorService.replace_vars(body_json_str, current_path_vars)
                                            else:
                                                body = MobileExecutorService.replace_vars(str(body_raw), current_path_vars)
    ...
    def execute_feature_group(db: Session, feature_id: int, env_id: int, company_id: int, schedule_id: int = None, user_id: int = 1, max_concurrency: int = None, flow_type: str = 'api', capture_video: bool = False, capture_screenshot: bool = False, visible_execution: bool = False, dataset_row: dict = None, failed_item_ids: list = None):
        ...
        variables = MobileExecutorService.get_merged_variables(db, feature.product_id, env_id)
        ...
        if flows:
            ...
            def process_feature_run(idx):
                ...
                try:
                    ...
                    s, f = MobileExecutorService.execute_flow_logic(
    ...
    def execute_flow_by_id(db: Session, flow_id: int, env_id: int, company_id: int, schedule_id: int = None, user_id: int = 1, max_concurrency: int = None, flow_type: str = 'api', capture_video: bool = False, capture_screenshot: bool = False, visible_execution: bool = False, dataset_row: dict = None):
        ...
        # 4. Prepare Variables
        variables = MobileExecutorService.get_merged_variables(db, product_id, env_id)
