...
def bulk_create_variables(vars: List[VariableCreate], db: Session = Depends(get_db)):
    return VariableService.bulk_create(db, vars)


@router.put("/variables/{var_id}", response_model=VariableResponse)
def update_variable(var_id: int, var: VariableCreate, db: Session = Depends(get_db)):
    try:
