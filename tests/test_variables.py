...
def test_create_and_update_variable(client):
    ...
    proj = client.post("/projects/", headers=headers, json={"name": "Var Proj"}).json()
...
def test_delete_variable(client):
    ...
    proj = client.post(
        "/projects/", headers=headers, json={"name": "Del Var Proj"}
    ).json()

    var = client.post(
        "/variables", json={"name": "TO_DEL", "value": "1", "project_id": proj["id"]}
    ).json()

    # Delete by ID
    resp = client.delete(f"/variables/{var['id']}")
    assert resp.status_code == 200

    # Verify
    list_resp = client.get(f"/variables?project_id={proj['id']}")
...
def test_delete_variable_by_name(client):
    ...
    proj = client.post(
        "/projects/", headers=headers, json={"name": "Del Name Proj"}
    ).json()

    client.post(
        "/variables", json={"name": "DEL_ME", "value": "1", "project_id": proj["id"]}
    )

    resp = client.delete(f"/variables/by-name/DEL_ME?project_id={proj['id']}")
    assert resp.status_code == 200
...
def test_bulk_create_variables(client):
    ...
    proj = client.post("/projects/", headers=headers, json={"name": "Bulk Proj"}).json()
    ...
    resp = client.post("/variables/bulk", json=vars_list)
    assert resp.status_code == 200
